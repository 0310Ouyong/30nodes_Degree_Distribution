...
total_proportion <- c()
for(j in 1:112){
  data <- filter(EcoBuilder_Degree,group==j)
  proportion <- c()
...
EcoBuilder_Degree$y <- total_proportion
EcoBuilder_Degree[,1] <- log10(EcoBuilder_Degree$x)
